...
static struct clk_lookup msm_clocks_8974[] = {
	...
	CLK_LOOKUP("ref_clk",	cxo_a2.c,	"6-0028"),
...
{
#if defined(CONFIG_ARCH_MSM8974_APOLLO) || defined(CONFIG_ARCH_MSM8974_THOR)
	struct clk_lookup *ffc_src_clk = NULL, *ffc_clk = NULL;
#endif
#if defined(CONFIG_ARCH_MSM8974_APOLLO)
	struct clk_lookup *rfc_src_clk = NULL, *rfc_clk = NULL;
#endif
/* front camera */
#if defined(CONFIG_ARCH_MSM8974_APOLLO) || defined(CONFIG_ARCH_MSM8974_THOR)
	if (board_is_thor() || board_is_apollo()) {
		printk(KERN_INFO"detected thor/apollo config for camera\n");
		if (socinfo_get_version() >= 0x20000)
			ffc_src_clk = clkdev_alloc(&gp1_clk_src.c, "cam_src_clk", "20.qcom,camera");
		else
			ffc_src_clk = clkdev_alloc(&mclk2_clk_src.c, "cam_src_clk", "20.qcom,camera");

	}
	else
		ffc_src_clk = clkdev_alloc(&mclk0_clk_src.c, "cam_src_clk", "20.qcom,camera");

	if (!ffc_src_clk)
		printk(KERN_ERR"%s: failed to alloc ffc_src_clk\n", __func__ );
	else
		clkdev_add(ffc_src_clk);

	if (board_is_thor() || board_is_apollo()) {
		printk(KERN_INFO"detected thor/apollo config for camera\n");
		if (socinfo_get_version() >= 0x20000)
			ffc_clk = clkdev_alloc(&gcc_gp1_clk.c, "cam_clk", "20.qcom,camera");
		else
			ffc_clk = clkdev_alloc(&camss_mclk2_clk.c, "cam_clk", "20.qcom,camera");

	}
	else
		ffc_clk = clkdev_alloc(&camss_mclk0_clk.c, "cam_clk", "20.qcom,camera");
	if (!ffc_clk)
		printk(KERN_ERR"%s: failed to alloc ffc_clk\n", __func__);
	else
		clkdev_add(ffc_clk);
#endif
/* back camera */
#if defined(CONFIG_ARCH_MSM8974_APOLLO)
	if (socinfo_get_version() >= 0x20000)
		rfc_src_clk = clkdev_alloc(&mmss_gp0_clk_src.c, "cam_src_clk", "6c.qcom,camera");
	else
		rfc_src_clk = clkdev_alloc(&mclk0_clk_src.c, "cam_src_clk", "6c.qcom,camera");

	if (!rfc_src_clk)
		printk(KERN_ERR"%s: failed to alloc rfc_src_clk\n", __func__);
	else
		clkdev_add(rfc_src_clk);

	if (socinfo_get_version() >= 0x20000)
		rfc_clk = clkdev_alloc(&camss_gp0_clk.c, "cam_clk", "6c.qcom,camera");
	else
		rfc_clk = clkdev_alloc(&camss_mclk0_clk.c, "cam_clk", "6c.qcom,camera");
	if (!rfc_clk)
		printk(KERN_ERR"%s: failed to alloc rfc_clk\n", __func__);
	else
		clkdev_add(rfc_clk);
#endif
	if (SOCINFO_VERSION_MAJOR(socinfo_get_version()) == 2) {

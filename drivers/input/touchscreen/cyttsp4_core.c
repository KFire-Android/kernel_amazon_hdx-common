...
{
	device_remove_file(&(dev->dev), &dev_attr_drv_ver);
	device_remove_file(&(dev->dev), &dev_attr_drv_stat);
	device_remove_file(&(dev->dev), &dev_attr_ic_ver);
	device_remove_file(&(dev->dev), &dev_attr_ic_ver_raw);
#if defined(CY_USE_FORCE_LOAD) || defined(CONFIG_TOUCHSCREEN_DEBUG)
	device_remove_file(&(dev->dev), &dev_attr_ic_reflash);
#endif

#ifdef CONFIG_TOUCHSCREEN_DEBUG
	device_remove_file(&(dev->dev), &dev_attr_ic_grpnum);
	device_remove_file(&(dev->dev), &dev_attr_ic_grpoffset);
	device_remove_file(&(dev->dev), &dev_attr_ic_grpdata);
	device_remove_file(&(dev->dev), &dev_attr_hw_reset);
	device_remove_file(&(dev->dev), &dev_attr_hw_recov);
#endif /* --CONFIG_TOUCHSCREEN_DEBUG */

#ifdef CY_USE_REG_ACCESS
	device_remove_file(&(dev->dev), &dev_attr_drv_rw_regid);
	device_remove_file(&(dev->dev), &dev_attr_drv_rw_reg_data);
#endif
#ifdef JEM_FTM
	device_remove_file(&(dev->dev), &dev_attr_ftm_test);
#endif
#ifdef CONFIG_TOUCHSCREEN_DEBUG_ENABLE_ENTRY
	device_remove_file(&(dev->dev), &dev_attr_ts_debug);
#endif
}
...
void *cyttsp4_core_init(struct cyttsp4_bus_ops *bus_ops,
	struct device *dev, int irq, char *name,void *cyttsp4_core_platform_data)
{
	unsigned long irq_flags = 0;
	int i = 0;
	int min = 0;
	...
	ts->dev = dev;
	ts->bus_ops = bus_ops;
	ts->platform_data = (struct touch_platform_data*)cyttsp4_core_platform_data;
...
//	ts->bus_ops->tsdebug = CY_DBG_LVL_3;
#endif /* --CONFIG_TOUCHSCREEN_DEBUG */
	...
	snprintf(ts->phys, sizeof(ts->phys)-1, "%s", CY_I2C_PHYS_NAME);
	//ts->phys = CY_I2C_PHYS_NAME;
	input_device->phys = ts->phys;
#endif
...
#if 1 //Register the device after all the initializations are complete
	retval = input_register_device(input_device);
	if (retval < 0) {
		dev_err(ts->dev,
			"%s: Error, failed to register input device r=%d\n",

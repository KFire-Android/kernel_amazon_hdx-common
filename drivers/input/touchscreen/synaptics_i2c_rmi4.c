...
#define SYNAPTICS_TIME_RESET 10
...
//fengy	input_mt_sync_frame(rmi4_data->input_dev);
	input_mt_sync(rmi4_data->input_dev);
#endif
		...
		bool enable)
{
	...
	dev_info(&(rmi4_data->input_dev->dev), "start function synaptics_rmi4_irq_enable\n");
	if (enable) {
...
#ifdef CONFIG_OF
static int synaptics_rmi4_parse_dt(struct device *dev, struct synaptics_rmi4_platform_data *pdata)
{
	int retval;
	int tempval;
	struct device_node *np = dev->of_node;
	...
	/* read panel xy */
	retval = of_property_read_u32(np, "synaptics,panel-x", &tempval);
	if (retval) {
		dev_err(dev, "%s: read panel-x fail\n", __func__);
		return retval;
	} else
		pdata->panel_x = tempval;

	retval = of_property_read_u32(np, "synaptics,panel-y", &tempval);
	if (retval) {
		dev_err(dev, "%s: read panel-y fail\n", __func__);
		return retval;
	} else
		pdata->panel_y = tempval;
	...
	*/
	...
	/* reset, irq gpio info */
	pdata->gpio_reset = of_get_named_gpio_flags(np, "synaptics,reset-gpio",
				0, &pdata->gpio_reset_flags);
	pdata->gpio_irq = of_get_named_gpio_flags(np, "synaptics,irq-gpio",
				0, &pdata->gpio_irq_flags);

	dev_info(dev, "%s: panel x %d y %d, reset gpio %d, irq gpio %d\n", \
		__func__, pdata->panel_x, pdata->panel_y, pdata->gpio_reset, pdata->gpio_irq);
		...
		const struct i2c_device_id *dev_id)
{
	...
	struct synaptics_rmi4_data *rmi4_data;
	struct synaptics_rmi4_device_info *rmi;
	struct synaptics_rmi4_platform_data *platform_data = client->dev.platform_data;
	...
	if (IS_ERR(rmi4_data->regulator)) {
		rc = PTR_ERR(rmi4_data->regulator);
		dev_err(&client->dev,
	...
	if (regulator_count_voltages(rmi4_data->regulator) > 0) {
		rc = regulator_set_voltage(rmi4_data->regulator, 3000000, 3000000);
		if (rc) {
			dev_err(&client->dev,
	...
	//interrupt
	if (gpio_is_valid(platform_data->gpio_irq)) {
		retval = gpio_request(platform_data->gpio_irq, "touch_interrupt");
		if (retval) {
			pr_err("request irq gpio failed, rc=%d\n", retval);
			gpio_free(platform_data->gpio_irq);
			return -ENODEV;
		}
		retval = gpio_direction_input(platform_data->gpio_irq);
		if (retval) {
			pr_err("set_direction for touch irq gpio failed, rc=%d\n",
				retval);
			gpio_free(platform_data->gpio_irq);
			return -ENODEV;
		}
	...
	if (gpio_is_valid(platform_data->gpio_reset)) {
		retval = gpio_request(platform_data->gpio_reset, "touch_reset");
		if (retval) {
			pr_err("request reset gpio failed, rc=%d\n", retval);
			gpio_free(platform_data->gpio_reset);
			return -ENODEV;
		}
		retval = gpio_direction_output(platform_data->gpio_reset, 0);
		if (retval) {
		...
		msleep(SYNAPTICS_TIME_RESET);
		retval = gpio_direction_output(platform_data->gpio_reset, 1);
		if (retval) {
			pr_err("set_direction for touch reset gpio failed, rc=%d\n",
				retval);
			gpio_free(platform_data->gpio_reset);
	...
	dev_info(&client->dev, "synaptics query device ok\n");
	...
	rmi4_data->irq = gpio_to_irq(platform_data->gpio_irq);
	platform_data->irq_type = IRQF_TRIGGER_LOW | IRQF_ONESHOT;

	dev_info(&client->dev,"setting irq. GPIO %d, irq %d, type %d\n", 
			platform_data->gpio_irq, rmi4_data->irq, 
...
static void synaptics_rmi4_sensor_sleep(struct synaptics_rmi4_data *rmi4_data)
{
	...
	dev_info(&(rmi4_data->input_dev->dev), "synaptics sleep\n");
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			rmi4_data->f01_ctrl_base_addr,
			&device_ctrl,
			sizeof(device_ctrl));
...
static void synaptics_rmi4_sensor_wake(struct synaptics_rmi4_data *rmi4_data)
{
	...
	dev_info(&(rmi4_data->input_dev->dev), "synaptics wake\n");
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			rmi4_data->f01_ctrl_base_addr,
			&device_ctrl,
			sizeof(device_ctrl));
...
static void synaptics_rmi4_early_suspend(struct early_suspend *h)
{
	...
	dev_info(&(rmi4_data->input_dev->dev), "synaptics early suspend\n");
	rmi4_data->touch_stopped = true;
	wake_up(&rmi4_data->wait);
...
static void synaptics_rmi4_late_resume(struct early_suspend *h)
{
	...
	dev_info(&(rmi4_data->input_dev->dev), "synaptics late resume\n");
	if (rmi4_data->full_pm_cycle)
		synaptics_rmi4_resume(&(rmi4_data->input_dev->dev));
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	dev_info(&(rmi4_data->input_dev->dev), "synaptics suspend\n");

...
#define STATUS_WORK_INTERVAL 10 /* ms */
#define WATCHDOG_HRTIMER 1
...
static void timeout_set_status(struct work_struct *work)
{
	...
	if (f54->status == STATUS_BUSY) {
		pr_info("%s: Get report command timed out\n", __func__);
		retval = f54->fn_ptr->read(rmi4_data,
...
static void print_image_report(void)
{
	...
	case F54_FULL_RAW_CAP_RX_COUPLING_COMP:
		...
		for (i = 0; i < f54->rmi4_data->num_of_tx; i++) {
			for (j = 0; j < f54->rmi4_data->num_of_rx; j++) {
				c[0] = f54->report_data[k];
				c[1] = f54->report_data[k + 1];
				memcpy(&s, &c, 2);
				if (s < -64)
					pr_info(".");
				else if (s < 0)
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!is_report_type_valid(setting)) {
		dev_err(&rmi4_data->i2c_client->dev,
...
MODULE_DESCRIPTION("RMI4 F54 Module");
MODULE_LICENSE("GPL");

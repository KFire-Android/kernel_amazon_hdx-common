	...
	struct v4l2_buffer *pb)
{
	...
	long rc;
	struct msm_session *session;
	struct msm_sensor_metadata* usr_ptr = NULL;
	struct msm_sensor_metadata* kernel_ptr = NULL;
	uint32_t magic;
	int skip_usr_cpy = 1;
	struct camera_v4l2_private *sp = fh_to_private(fh);
		struct msm_video_device *pvdev = video_drvdata(filep);
	...
	/* Validate if we have a valid user pointer put into pb->reserved by
	 * checking for the desired magic */
	usr_ptr =(struct msm_sensor_metadata*) pb->reserved;
	if(usr_ptr)
	{
		rc = copy_from_user(&magic, usr_ptr, sizeof(uint32_t));
		if(rc)
		{
			pr_err("%s copy_from_user failed %ld copied %d expected",
				__func__,rc, sizeof(uint32_t));
			return -EINVAL;
		}
		if(magic == MSM_SENSOR_METADATA_MAGIC)
			skip_usr_cpy = 0;
	}

	/* Its a bit tricky here, before call to vb2_dqbuf, pb->reserved points
	 * to usr space metadata ptr. After the call to vb2_dqbuf, the vb2_buffer
	 * information gets copied to v4l2_buffer that arrived from user space and
	 * hence pb->reserved would point to the kernel space  metadata pointer. It
	 * looks hacky but this was the only way to accomplish goal by not touching
	 * v4l2 code */

	mutex_lock(&session->lock);
	ret = vb2_dqbuf(&sp->vb2_q, pb, filep->f_flags & O_NONBLOCK);
	mutex_unlock(&session->lock);
	/* if vb2_dqbuf failed, return right away*/
	if(ret)
		return ret;

	/* right after vb2_dqbuf, we have the reserved field now populated with
	 * the kernel metadata ptr, we read magic to make sure this has happened */
	kernel_ptr = (struct msm_sensor_metadata*) pb->reserved;
	if(!skip_usr_cpy && kernel_ptr && (kernel_ptr->magic == MSM_SENSOR_METADATA_MAGIC))
	{
		rc = copy_to_user(usr_ptr, kernel_ptr, sizeof(struct msm_sensor_metadata));
		if(rc)
		{
			pr_err("%s copy_to_user failed %ld copied %d expected",
				__func__, rc, sizeof(struct msm_sensor_metadata));
			return -EINVAL;
		}
		kernel_ptr->magic = 0;
	}

	...
	struct timeval *tv, uint32_t frame_id, uint32_t output_format)
{
	...
	/* metadata struct is contained under the impl specific msm_vb2_buffer
	 * which also contians struct vb2_buffer */
	msm_vb2 = container_of(buf_info->vb2_buf, struct msm_vb2_buffer, vb2_buf);
	...
	if (state == MSM_ISP_BUFFER_STATE_DEQUEUED ||
		...
		if ((BUF_SRC(bufq->stream_id))) {
			...
			/* call sensor metadata hook based on session id */
			sensor_subdev = msm_sensor_sd_find(bufq->session_id);
			if(sensor_subdev){
				rc = v4l2_subdev_call(sensor_subdev, core, ioctl,
					VIDIOC_SENSOR_GET_METADATA,
					&msm_vb2->metadata);
				if(rc != 0)
					msm_vb2->metadata.output_format = output_format;
			}
			else
				msm_vb2->metadata.output_format = output_format;

			/* Assign the metadata back to the reserved field also
			 * setting magic so that the ioctl code knows that this has been done
			 * vb2_buffer.reserved so that it may be accessed later in dqbuf ioctl, we need
			 * to do this as we do not have access to struct vb2_buffer in the dqbuf ioctl
			 **/
			msm_vb2->metadata.magic = MSM_SENSOR_METADATA_MAGIC;
			buf_info->vb2_buf->v4l2_buf.reserved = (uint32_t)&msm_vb2->metadata;

			/* call vb2_buf_done to return the buffer to done_list */

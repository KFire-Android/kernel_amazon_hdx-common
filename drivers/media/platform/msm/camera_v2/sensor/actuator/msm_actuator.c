...
static int32_t msm_ois_init(struct msm_actuator_ctrl_t *a_ctrl)
{
	uint8_t data[2];
	...
	char *fw_name_prog = "bu63164.prog";
	char *fw_name_coeff = "bu63164.coeff";
	...
	if (rc) {
			...
			goto msm_ois_init_Exit;
	}

	TotalBytes = fw->size;
	pr_err("rajeshb in %s at line %d totalbytes = %d\n", __func__, __LINE__, TotalBytes);
	...
	rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_read_seq(
		&a_ctrl->i2c_client,
		(OpCode_Memory_RW << 8) | 0xF7, data, 2);
	if (rc < 0){
		goto msm_ois_init_Exit;
	}
	...
	if (data[1] != 0x04)
	{
		pr_err("%d in %s, Firmware download failed", __LINE__, __func__);
		rc = -EFAULT;
		goto msm_ois_init_Exit;
	}
	...
	{
		BytesInTx = (TotalBytes >= 10) ? 10 : TotalBytes;
		pr_err("rajeshb in %s at line %d BytesInTx = %d\n", __func__, __LINE__, BytesInTx);
		rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_write_seq(
			&a_ctrl->i2c_client,
			OpCode_Coeff, ptr,
			BytesInTx);
	...
	return 0;
msm_ois_init_Exit:
	pr_err("rajeshb in %s at line %d\n", __func__, __LINE__);
	return rc;

}

static int32_t msm_ois_fact_adj_apply(
	...
	uint16_t total_entries)
{
	...
	for (i = 0; i < total_entries; i++)
	{
		...
		if (rc < 0)
			goto msm_ois_fact_adj_apply_Exit;
		pFactoryAdjData[i].data = (data[1] << 8) | data[0];

		a_ctrl->i2c_client.cci_client->sid = sensor_sid;

		rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_write_seq(
			&a_ctrl->i2c_client,
			(OpCode_Pheripheral_RW << 8) | pFactoryAdjData[i].ctrl_addr,
			data, 2);
...
msm_ois_fact_adj_apply_Exit:
	pr_err("rajeshb in %s at line %d rc=%d\n", __func__, __LINE__, rc);
	return rc;
	...
	struct reg_settings_t *settings)
{
	...
	rc = msm_ois_fact_adj_apply(a_ctrl, FactoryAdjData, sizeof(FactoryAdjData) / sizeof(FactoryAdjData[0]));
	if (rc < 0)
		return rc;

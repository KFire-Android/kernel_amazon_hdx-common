	...
	void *data)
{
	...
	case MSM_CAMERA_LED_LOW:
		if (fctrl->flash_trigger[2]) {
                        current_l = cfg->led_current ?
                                cfg->led_current : fctrl->flash_op_current[2];
                        led_trigger_event(fctrl->flash_trigger[2], current_l);
                }
	...
	case MSM_CAMERA_LED_HIGH:
		...
		for (i = 0; i < fctrl->num_sources; i++) {
			if (fctrl->flash_trigger[i]) {
				current_l = cfg->led_current ?
	                                cfg->led_current : fctrl->flash_op_current[i];

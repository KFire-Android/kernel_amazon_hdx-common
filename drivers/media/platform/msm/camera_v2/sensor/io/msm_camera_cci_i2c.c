...
EXPORT_SYMBOL_GPL(msm_camera_cci_i2c_read);
...
EXPORT_SYMBOL_GPL(msm_camera_cci_i2c_write);
	...
	for (i = 0; i < size; i++) {
		...
		if (reg_conf_tbl->cmd_type == MSM_CAMERA_I2C_CMD_POLL) {
			rc = msm_camera_cci_i2c_poll(client,
				...
				reg_conf_tbl->dt);
		} else if ( reg_conf_tbl->cmd_type == MSM_CAMERA_I2C_CMD_CB) {
			if ( client->write_op_cb != NULL ) {
                            rc = client->write_op_cb(client, reg_conf_tbl);
			}
		} else {
...
EXPORT_SYMBOL_GPL(msm_sensor_cci_i2c_util);

...
struct camera_vreg_t cam_extra_vreg = {
	.reg_name = "cam_extra",
	.type = REG_VS,
	.min_voltage = 0,
	.max_voltage = 0,
...
struct  regulator *cam_extra_reg;
...
int32_t msm_sensor_free_sensor_data(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	kfree(s_ctrl->clk_info2);
	return 0;
}

EXPORT_SYMBOL_GPL(msm_sensor_free_sensor_data);
...
static struct msm_cam_clk_info cam_8974_clk_info2[] = {
	[SENSOR_CAM_MCLK] = {"cam_src_clk2", 19200000},
	[SENSOR_CAM_CLK] = {"cam_clk2", 0},
};
...
int32_t msm_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	for (index = 0; index < power_setting_array->size; index++) {
		...
		case SENSOR_CLK:
			...
			break;
		case SENSOR_CLK2:
			if (power_setting->seq_val >= s_ctrl->clk_info_size2) {
				pr_err("%s clk index %d >= max %d\n", __func__,
					power_setting->seq_val,
					s_ctrl->clk_info_size2);
				goto power_up_failed;
			}
			if (power_setting->config_val)
				s_ctrl->clk_info2[power_setting->seq_val].
					clk_rate = power_setting->config_val;
		...
		case SENSOR_VREG:
			...
			if (power_setting->seq_val == CAM_VANA) {
				/* extra camera supply needed for galvatron  */
				msm_camera_config_single_vreg(s_ctrl->dev,
						&cam_extra_vreg,
						&cam_extra_reg, 1);
...
EXPORT_SYMBOL_GPL(msm_sensor_power_up);
...
EXPORT_SYMBOL_GPL(msm_sensor_power_down);
...
EXPORT_SYMBOL_GPL(msm_sensor_subdev_ioctl);
...
EXPORT_SYMBOL_GPL(msm_sensor_power);
...
int32_t msm_sensor_platform_probe(struct platform_device *pdev, void *data)
{
	...
	s_ctrl->clk_info2 = kzalloc(sizeof(cam_8974_clk_info2),
		GFP_KERNEL);
	if (!s_ctrl->clk_info2) {
		pr_err("%s:%d failed nomem\n", __func__, __LINE__);
		kfree(cci_client);
	...
	memcpy(s_ctrl->clk_info2, cam_8974_clk_info2, sizeof(cam_8974_clk_info2));
	s_ctrl->clk_info_size2 = ARRAY_SIZE(cam_8974_clk_info2);
	...
	/* assign video dev number, session id to subdev video_device */
	s_ctrl->msm_sd.sd.devnode->num = session_id;
	CDBG("%s:%d\n", __func__, __LINE__);
	...
	if(ret)
		...
		msm_sensor_sysfs_file_create(&pdev->dev);

	CDBG("%s:%d\n", __func__, __LINE__);
	return rc;
}

EXPORT_SYMBOL_GPL(msm_sensor_platform_probe);
...
EXPORT_SYMBOL_GPL(msm_sensor_i2c_probe);

...
static headset_plug_type_t current_plug_type = INPUT_TYPE_NONE;
...
int ts3a225e_headset_switch_init(void) {
    ...
    /* Set CTRL3 Register AUTO_SW_DIS to 0, to disable automatic detection
       ...
       set to the defaults.
     */
    result = ts3a225e_headset_switch_write_register(switch_dev_info, TS3A225E_CTRL3_REG_ADDR, 0x0E);
    if (result < 0) {
    ...
    /* Default to the CTIA configuration at powerup */

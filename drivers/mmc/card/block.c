...
#define MMC_BLK_SECDISCARD	BIT(3)
	...
	struct device_attribute encryption_state;
	struct device_attribute crypto_info;
	int	area_type;
};
...
static ssize_t
encryption_state_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct mmc_blk_data *md = mmc_blk_get(dev_to_disk(dev));
	struct mmc_card *card = md->queue.card;
	...
	ret = snprintf(buf, sizeof(card->encryption_state) + 1 , "%d",
						card->encryption_state);
...
static ssize_t
encryption_state_store(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t count)
{
	int value;
	...
	struct mmc_card *card = md->queue.card;
	int ret = count;
	int max_buf_size = sizeof(card->encryption_state) + 1;
	...
	if ((value >= MMC_NO_ENCRYPTION) &&
		(value < MMC_MAX_DISK_ENCRYPTION_STATE)) {
		card->encryption_state = value;
		pr_debug("%s:%s encryption_state = %d\n",
...
static ssize_t
crypto_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mmc_blk_data *md = mmc_blk_get(dev_to_disk(dev));
	struct mmc_card *card = md->queue.card;
	int ret;
	...
	ret = snprintf(buf, MMC_MAX_CRYPTO_INFO_SIZE , "%s",
						card->crypto_info);
...
static ssize_t
crypto_info_store(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t count)
{
	struct mmc_blk_data *md = mmc_blk_get(dev_to_disk(dev));
	struct mmc_card *card = md->queue.card;
	int ret = count;
	int buf_len = MMC_MAX_CRYPTO_INFO_SIZE;
	...
	/* Make sure there is no buffer overflow */
	if (buf)
		buf_len = strnlen(buf, MMC_MAX_CRYPTO_INFO_SIZE);

	if (buf_len >= MMC_MAX_CRYPTO_INFO_SIZE) {
		ret = -EINVAL;
		goto exit;
	}

	/* Now we can safely copy the buffer */
	strlcpy(card->crypto_info, buf, buf_len + 1);
	pr_debug("%s:%s crypto_info = %s\n", mmc_hostname(card->host),
					__func__, card->crypto_info);
...
static u8 mmc_blk_prep_packed_list(struct mmc_queue *mq, struct request *req)
{
	...
	if (mmc_should_encrypt_decrypt(card->host, cur))
		goto no_packed;
	...
	while (reqs < max_packed_rw - 1) {
		...
		if (mmc_should_encrypt_decrypt(card->host, next)) {
			MMC_BLK_UPDATE_STOP_REASON(stats, ENCRYPT);
			put_back = 1;
			break;
		}
...
static int mmc_add_disk(struct mmc_blk_data *md)
{
	...
	md->encryption_state.show = encryption_state_show;
	md->encryption_state.store = encryption_state_store;
	sysfs_attr_init(&md->encryption_state.attr);
	md->encryption_state.attr.name = "encryption_state";
	md->encryption_state.attr.mode = S_IRUGO | S_IWUSR;
	ret = device_create_file(disk_to_dev(md->disk),
				&md->encryption_state);
	if (ret)
		goto encryption_state_fails;

	md->crypto_info.show = crypto_info_show;
	md->crypto_info.store = crypto_info_store;
	sysfs_attr_init(&md->crypto_info.attr);
	md->crypto_info.attr.name = "crypto_info";
	md->crypto_info.attr.mode = S_IRUGO | S_IWUSR;
	ret = device_create_file(disk_to_dev(md->disk),
				&md->crypto_info);
	if (ret)
		goto crypto_info_fails;

...
static inline int bwan_on(int retry_count)
{
	...
	/* BWAN_HMI is now dual-purpose. During boot it controls entry
	into fastboot (0 = fastboot, 1 = normal boot). After boot (FW_READY asserted)
	the GPIO is used to wake up the modem from XO shutdown. */
	printk ("BWAN_HMI=1\n");
	gpio_direction_output(BWAN_HMI, 1);
	...
	do {
		...
		if (retval > 0) {
			if (!bwan_fw_rdy_status) {
				...
				gpio_direction_output(BWAN_HMI, 0);
			}
			break;
		} else {
...
static ssize_t bwan_fastboot_store(struct kobject *kobj,
				struct kobj_attribute *attr,
				const char *buf,
				size_t count)
{
	...
	if (strcmp(attr->attr.name, "fastboot")) {
		return -1;
	}
	...
	switch (var) {
		case 1:
			...
			bwan_pulse_gpio_wan_shutdown(POWER_OFF_HOLD_TIME);
			msleep(1000);
			printk ("fastboot mode: BWAN_HMI=0\n");
			gpio_direction_output(BWAN_HMI, 0);
			bwan_pulse_gpio_wan_on(POWER_ON_HOLD_TIME);
...
static struct kobj_attribute bwan_fastboot_attribute =
	__ATTR(fastboot, 0222, NULL, bwan_fastboot_store);

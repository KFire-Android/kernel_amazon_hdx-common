...
//	struct regulator	*pmuvcc_reg;
	...
	bool				clk_enabled;
	spinlock_t			clk_enabled_lock;
};

/*
What we want to do if clk_enable/disable() actually works....
...
What we do instead: push clock requests to a work queue, which calls clk_prepare/unprepare()
-sus
*/
static struct workqueue_struct *clk_ctl_wq;

typedef struct {
	struct work_struct clk_ctl;
	struct pn544_dev *pn544_dev;
} clk_ctl_t;

clk_ctl_t *clk_ctl_workitem;

static void clk_ctl_wq_func(struct work_struct *work)
{
	clk_ctl_t *clk_ctl = (clk_ctl_t *)work;

	if(gpio_get_value(clk_ctl->pn544_dev->clk_req_gpio)) {
		if(!clk_ctl->pn544_dev->clk_enabled) {
			clk_prepare(clk_ctl->pn544_dev->ref_clk);
			clk_ctl->pn544_dev->clk_enabled = true;
		}
	} else {
		if(clk_ctl->pn544_dev->clk_enabled) {
			clk_unprepare(clk_ctl->pn544_dev->ref_clk);
			clk_ctl->pn544_dev->clk_enabled = false;
		}
	}
...
static irqreturn_t pn544_dev_clk_req_irq_handler(int irq, void *dev_id)
{
	clk_ctl_workitem = (clk_ctl_t *)kmalloc(sizeof(clk_ctl_t), GFP_ATOMIC);
	if(clk_ctl_workitem) {
		INIT_WORK((struct work_struct *)clk_ctl_workitem, clk_ctl_wq_func);

		clk_ctl_workitem->pn544_dev = dev_id;

		queue_work(clk_ctl_wq, (struct work_struct *)clk_ctl_workitem);
	}
...
static int pn544_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if((int)platform_data->clk_req_gpio > 0) {
		pr_info("%s have clk request pin: prepare clk\n", __func__);
//clk_prepare actually enables the clock as well, so skip it for now. -sus
//		clk_prepare(pn544_dev->ref_clk);
		pn544_dev->clk_enabled = false;
	} else {
		/* if the clock request pin is not present, we have to permanently enable the ref clock. */
		pr_info("%s no clk request pin: prepare & enable clk\n", __func__);
	...
	if((int)platform_data->clk_req_gpio > 0) {
		clk_ctl_wq = create_workqueue("clk_ctl_queue");
		if(clk_ctl_wq) {
			...
			pr_err("%s : unable to create clock control workqueue; clock request IRQ not requested\n", __func__);
		}
...
static int pn544_remove(struct i2c_client *client)
{
	...
	if(clk_ctl_wq) {
		flush_workqueue(clk_ctl_wq);
		destroy_workqueue(clk_ctl_wq);
	}

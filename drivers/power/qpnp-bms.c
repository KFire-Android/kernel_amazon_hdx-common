...
#if defined(CONFIG_BQ27541_GG)
	union power_supply_propval value;
	value.intval = report_state_of_charge(chip);
	if (bq27541_get_property_backdoor(POWER_SUPPLY_PROP_CAPACITY, &value))
		return 50;
	return value.intval;
#else
...
static int procfs_read(char *pPage, char **ppStart, off_t off, int count,
		int *pEof, void *pData)
{
	struct qpnp_bms_chip *chip = (struct qpnp_bms_chip *)pData;
	int i, length = 0;
	...
	length += sprintf(pPage + length,
			...
			"adjust_soc_low_threshold: %d\n"
			"chg_term_ua: %d\n"
			"batt_type: %d\n"
			"fcc: %u\n"
			"default_rbatt_mohm: %d\n",
			chip->r_sense_uohm/1000,
			chip->v_cutoff_uv,
			chip->max_voltage_uv,
			chip->r_conn_mohm,
			chip->shutdown_soc_valid_limit,
	...
	length += sprintf(pPage + length,
			"use_external_rsense: %d\n"
			"use_ocv_thresholds: %d\n"
			"ignore_shutdown_soc: %d\n"
			"shutdown_soc_invalid: %d\n"
			"shutdown_soc: %d\n"
			"shutdown_iavg_ma: %d\n",
	...
	length += sprintf(pPage + length,
			"ocv_reading_at_100: %u\n"
			"prev_last_good_ocv_raw: %u\n"
			"last_ocv_uv: %d\n"
			"last_cc_uah: %d\n"
			"tm_sec: %lu\n"
			"first_time_calc_soc: %d\n"
			"first_time_calc_uuc: %d\n",
			chip->ocv_reading_at_100,
			chip->prev_last_good_ocv_raw,
			chip->last_ocv_uv,
			chip->last_cc_uah,
			chip->tm_sec,
			chip->first_time_calc_soc,
			chip->first_time_calc_uuc);

	length += sprintf(pPage + length, "iavg_samples_ma[]:");
	for (i = 0; i < IAVG_SAMPLES; i++)
		length += sprintf(pPage + length, " %d",
				chip->iavg_samples_ma[i]);
	pPage[length++] = '\n';
	...
	length += sprintf(pPage + length,
			"iavg_index: %d\n"
			"iavg_num_samples: %d\n"
			"t_soc_queried: %ld.%09ld\n"
			"last_soc: %d\n"
			"last_soc_est: %d\n",
			chip->iavg_index,
			chip->iavg_num_samples,
			chip->t_soc_queried.tv_sec, chip->t_soc_queried.tv_nsec,
			chip->last_soc,
			chip->last_soc_est);
	...
	length += sprintf(pPage + length,
			...
			"prev_chg_soc: %d\n"
			"calculated_soc: %d\n"
			"prev_voltage_based_soc: %d\n"
			"use_voltage_soc: %d\n"
			"ocv_high_threshold_uv: %d\n"
			"ocv_low_threshold_uv: %d\n",
...
static ssize_t show_use_voltage_soc(struct device *dev,
...
static ssize_t set_use_voltage_soc(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
	struct qpnp_bms_chip *chip = dev_get_drvdata(dev);
	chip->use_voltage_soc = buf[0] == '1';
	power_supply_changed(&chip->bms_psy);
	return count;
}

static DEVICE_ATTR(use_voltage_soc, S_IWUSR | S_IRUGO, show_use_voltage_soc,
						set_use_voltage_soc);
 ...
 */
{
	...
	if (bq27541_get_property_backdoor(
		...
		pr_info("[%d], %d, %d, %d, %d, %d, %d\n",
				...
				taper_ma, delta_mv, shutoff_mv);
		if (count == 6) {
			chip->v_cutoff_uv = shutoff_mv * 1000;
			chip->low_voltage_threshold = chip->v_cutoff_uv + 20;
			chip->max_voltage_uv = charge_mv * 1000;
			chip->chg_term_ua = (taper_ma < 100 ? 100 : taper_ma) * 1000;
			okay = 1;
		}
	}
	if (!okay) {
...
{
	union power_supply_propval value;
	rc = bq27541_get_property_backdoor(POWER_SUPPLY_PROP_CAPACITY, &value);
	if (rc == -EINVAL) {
...

	{
		struct proc_dir_entry *dent = create_proc_entry(
				"qpnp-bms", S_IFREG | 0444, NULL);
		if (dent) {
			dent->read_proc = procfs_read;
			dent->data = (void *)chip;
		}
		device_create_file(&spmi->dev, &dev_attr_use_voltage_soc);
	}

...
#define CONFIG_CHARGER_SNAPSHOT
...
#if defined(CONFIG_CHARGER_SNAPSHOT)
...
/**
 * This is a temporary backdoor into the GG driver. Access would normally be
 * through a registered power supply, but that is problematic, at this time.
 * Didn't have a header file to use, so simply added it here.
 */
extern int bq27541_get_property_backdoor(
...
#if defined(CONFIG_BQ27541_GG)
	union power_supply_propval value;
	if (bq27541_get_property_backdoor(POWER_SUPPLY_PROP_PRESENT, &value) ||
			value.intval == 0)
		return 0;
	return 1;
#else
...
#define PROTOCOL_DELAY()	udelay(200)		// QC
static int
...
//JHL check this - had been getting a storm of these things when the
// charger is enabled, and connected to a non-sinking power supply
static irqreturn_t
...
#if 0
// useful for tracing battery activity while debugging charging
{
	union power_supply_propval scope = {0}, online = {0}, imax = {0};
	if (chip->usb_psy) {
		chip->usb_psy->get_property(chip->usb_psy,
				POWER_SUPPLY_PROP_SCOPE, &scope);
		chip->usb_psy->get_property(chip->usb_psy,
				POWER_SUPPLY_PROP_ONLINE, &online);
		chip->usb_psy->get_property(chip->usb_psy,
				POWER_SUPPLY_PROP_CURRENT_MAX, &imax);
	}
	pr_info("usb: %d, bms: %d, scope: %d, online: %d, imax: %d, "
			"otg_en_set: %d, chg_plugged_in: %d, "
			"batt_present: %d\n",
			chip->usb_psy ? 1 : 0, chip->bms_psy ? 1 : 0,
			scope.intval, online.intval, imax.intval,
...
qpnp_chg_request_irqs(struct qpnp_chg_chip *chip)
{
	...
	spmi_for_each_container_dev(spmi_resource, chip->spmi) {
		...
		chip->chg_trklchg.irq = spmi_get_irq_byname(chip->spmi,
...
static int
procfs_read(char *pPage, char **ppStart, off_t off, int count,
		int *pEof, void *pData)
{
	struct qpnp_chg_chip *chip = (struct qpnp_chg_chip *)pData;

	return sprintf(pPage,
			"chgr_base: %u\n"
			"buck_base: %u\n"
			"bat_if_base: %u\n"
			"usb_chgpth_base: %u\n"
			...
			"dc_present: %d\n"
			"charging_disabled: %d\n"
			"use_default_batt_values: %d\n"
			"max_bat_chg_current: %u\n"
			"safe_voltage_mv: %u\n"
			"max_voltage_mv: %u\n"
			"min_voltage_mv: %u\n"
			"resume_delta_mv: %u\n"
...
#if defined(CONFIG_CHARGER_SNAPSHOT)
static void qpnp_chg_cancel_snapshot(struct qpnp_chg_chip * chip)
{
	cancel_delayed_work(&chip->snapshot.work);
}

static void qpnp_chg_schedule_snapshot(struct qpnp_chg_chip * chip)
{
	qpnp_chg_cancel_snapshot(chip);
	if (chip->snapshot.seconds)
		schedule_delayed_work(&chip->snapshot.work,
				(HZ)*chip->snapshot.seconds);
}

static void qpnp_chg_do_snapshot(struct qpnp_chg_chip * chip)
{
	u8 ibat, psrc, iusb;
	qpnp_chg_read(chip, &ibat, 0x1044, 1);
	qpnp_chg_read(chip, &psrc, 0x1308, 1);
	qpnp_chg_read(chip, &iusb, 0x1344, 1);
	pr_info("SOC %d, IBAT %d, VBAT %d, TEMP %d, USB %d, ICHG %d, PSRC %d, IUSB %d\n",
			...
			100 + (ibat & 0x3F) * 50,
			psrc & 0x03,
			iusb?(iusb>1?iusb*100:150):100);
}

static void qpnp_chg_snapshot(struct work_struct *work)
{
	struct qpnp_chg_chip *chip = container_of(work,
			struct qpnp_chg_chip, snapshot.work.work);
	qpnp_chg_do_snapshot(chip);
	qpnp_chg_schedule_snapshot(chip);
}

static ssize_t show_snapshot_seconds(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct qpnp_chg_chip *chip = dev_get_drvdata(dev);
	return sprintf(buf, "%d\n", chip->snapshot.seconds);
}

static ssize_t set_snapshot_seconds(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
	struct qpnp_chg_chip *chip = dev_get_drvdata(dev);
	long sec;
	if (kstrtol(buf, 10, &sec) == 0) {
		chip->snapshot.seconds = (unsigned int)(sec < 0 ? 0 : sec);
		qpnp_chg_schedule_snapshot(chip);
	}
	return count;
}

static DEVICE_ATTR(snapshot_seconds, S_IWUSR | S_IRUGO, show_snapshot_seconds,
						set_snapshot_seconds);
#endif
...
#if defined(CONFIG_BQ27541_GG)
/* Retrieve battery specs from the battery/GG driver.
 * Battery specs encoded in csv format in the MODEL property.
 */
{
	union power_supply_propval value;
	int okay = 0;
	value.strval = 0;
	if (bq27541_get_property_backdoor(
			POWER_SUPPLY_PROP_MODEL_NAME, &value) == 0 &&
			value.strval) {
		int capacity_mah, charge_mv, charge_rate, taper_ma,
				delta_mv, shutoff_mv;
		int count = sscanf(value.strval, "%d,%d,%d,%d,%d,%d",
				&capacity_mah, &charge_mv, &charge_rate,
				&taper_ma, &delta_mv, &shutoff_mv);
		pr_info("[%d], %d, %d, %d, %d, %d, %d\n",
				count, capacity_mah, charge_mv, charge_rate,
				taper_ma, delta_mv, shutoff_mv);
		if (count == 6) {
			chip->max_voltage_mv = chip->min_voltage_mv = chip->safe_voltage_mv = charge_mv;
			chip->safe_current = chip->max_bat_chg_current = (charge_rate * capacity_mah) / 10;
			chip->resume_delta_mv = delta_mv;
			chip->term_current = taper_ma < 100 ? 100 : taper_ma;
			//TODO cool/warm/thermal
			okay = 1;
		}
	}
	if (!okay) {
		pr_err("failed to retrieve POWER_SUPPLY_PROP_MODEL_NAME data\n");
		return -1;
	}
}
#endif
...
{
	union power_supply_propval value;
	rc = bq27541_get_property_backdoor(POWER_SUPPLY_PROP_PRESENT, &value);
	if (rc == -EINVAL) {
		pr_info("Battery/GG not ready, deferring probe\n");
		rc = -EPROBE_DEFER;
		goto fail_chg_enable;
	}
}
	...
	spmi_for_each_container_dev(spmi_resource, spmi) {
		...
		pr_debug("\"%s\", \"%s\"\n", spmi_resource->of_node->name,
				spmi_resource->label ? spmi_resource->label : "");
	...
	{
		struct proc_dir_entry *dent = create_proc_entry(
				"qpnp-chg", S_IFREG | 0444, NULL);
		if (dent) {
			dent->read_proc = procfs_read;
			dent->data = (void *)chip;
		}
	}
#if defined(CONFIG_CHARGER_SNAPSHOT)
	qpnp_chg_do_snapshot(chip);
	INIT_DELAYED_WORK(&chip->snapshot.work, qpnp_chg_snapshot);
	qpnp_chg_schedule_snapshot(chip);

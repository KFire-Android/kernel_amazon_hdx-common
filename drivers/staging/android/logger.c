...
#define LOGGER_LOG_SYSTEM_SIZE  (256*1024)
...
#ifdef CONFIG_AMAZON_METRICS_LOG
#define LOGGER_LOG_METRICS_SIZE (256*1024)
static unsigned char *_buf_log_metrics;
#endif

#ifdef CONFIG_AMAZON_LOG
#define LOGGER_LOG_AMAZON_MAIN_SIZE  (256*1024)
static unsigned char *_buf_log_amazon_main;
#endif
...
static int __init create_log(char *log_name, int size)
{
	...
	printk(KERN_INFO "logger: created %luK log '%s'\n",
        ...
        if (!strcmp(log_name,LOGGER_LOG_MAIN))
                _buf_log_main = buffer; 
        else if (!strcmp(log_name,LOGGER_LOG_EVENTS))
                _buf_log_events = buffer;
        else if (!strcmp(log_name,LOGGER_LOG_RADIO))
                _buf_log_radio  = buffer;
        else if (!strcmp(log_name,LOGGER_LOG_SYSTEM))
                _buf_log_system = buffer;  
#ifdef  CONFIG_AMAZON_METRICS_LOG
        else if (!strcmp(log_name,LOGGER_LOG_METRICS))
                _buf_log_metrics = buffer;
#endif
#ifdef CONFIG_AMAZON_LOG
        else if (!strcmp(log_name,LOGGER_LOG_AMAZON_MAIN))
                _buf_log_amazon_main = buffer;  
#endif
...
#ifdef CONFIG_AMAZON_METRICS_LOG
    ret = create_log(LOGGER_LOG_METRICS, LOGGER_LOG_METRICS_SIZE);
    if (unlikely(ret))
        goto out;
    metrics_init = 1;
#endif
...
#ifdef CONFIG_AMAZON_LOG
    ret = create_log(LOGGER_LOG_AMAZON_MAIN, LOGGER_LOG_AMAZON_MAIN_SIZE);
    if (unlikely(ret))
        goto out;
#endif

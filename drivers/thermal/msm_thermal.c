...
static DEFINE_PER_CPU(uint32_t, limited_min_freq);
               ...
               unsigned long event, void *data)
{
       ...
       int cpu = policy->cpu;
       unsigned int max_freq_req = per_cpu(limited_max_freq, policy->cpu);
       unsigned int min_freq_req = per_cpu(limited_min_freq, policy->cpu);

       switch (event) {
       case CPUFREQ_INCOMPATIBLE:
               pr_debug("%s: mitigating cpu %d to freq max: %u min: %u\n",
               KBUILD_MODNAME, cpu, max_freq_req, min_freq_req);

               cpufreq_verify_within_limits(policy, min_freq_req,
                       max_freq_req);

               if (max_freq_req < min_freq_req) {
                       pr_info("%s: WARNING: Invalid frequency limits. Max:%u Min:%u\n",
                               KBUILD_MODNAME, max_freq_req, min_freq_req);
               }
               break;
       }
       return NOTIFY_OK;
...
static int update_cpu_freq(int cpu)
{
       int ret = 0;
       ...
       if (cpu_online(cpu)) {
               if (cpufreq_update_policy(cpu))
                       pr_err("%s: Unable to update policy for cpu:%d\n",
                               KBUILD_MODNAME, cpu);
       }
       put_online_cpus();
	...
	min = min(min, table[limit_idx_high].frequency);
       ...
       if (freq_mitigation_task) {
               for_each_possible_cpu(cpu) {
                       mutex_lock(
                               &cpus[cpu].freq_mitig_data.freq_mitig_mutex);
                       cpus[cpu].freq_mitig_data.min_freq_limit = min;
                       mutex_unlock(
                               &cpus[cpu].freq_mitig_data.freq_mitig_mutex);
		}
	        complete(&freq_mitigation_complete);
               ...
               for_each_possible_cpu(cpu) {
                       per_cpu(limited_min_freq, cpu) = min;
                       update_cpu_freq(cpu);
		}
	}
...
{
	int cpu = 0;
        uint32_t max_freq = per_cpu(limited_max_freq, 0);
	...
	if (temp >= msm_thermal_info.limit_temp_degC) {
       ...
       if (max_freq == per_cpu(limited_max_freq, 0))
		return;

	/* Update new limits */
	for_each_possible_cpu(cpu) {
               per_cpu(limited_max_freq, cpu) = max_freq;
...
static void __ref temp_notify(struct work_struct *work)
{
	...
	char *thermsen_metrics = "thermsensor:def:thermonitor=1;CT;1";
#endif
	...
	for (i=0; i < ARRAY_SIZE(cpu_sensors); i++) {
		...
		if (ret) {
			...
			if (temp > 100) {
				...
				"%s,tsens_num=%d;CT;1,temp=%d;CT;1:NR",
				thermsen_metrics, tsens_dev.sensor_num,
							(int)temp);
...
#if defined(CONFIG_AMAZON_METRICS_LOG)
					snprintf(thermlog_buf,
						sizeof(thermlog_buf),
					"%s,tsens_num=%d;CT;1,threshold=%d;CT;1:NR",
					thermsen_metrics, tsens_dev.sensor_num,
						(int)temp_threshold);
...
static int set_threshold(char *sensor_type, struct sensor_threshold *threshold,
       int num_thresh)
{
       struct tsens_device tsens_dev;
       int i = 0, ret = 0;
       ...
       if (!sensor_type || !threshold || (num_thresh < 1)) {
               pr_err("%s: Invalid input\n", KBUILD_MODNAME);
               ret = -EINVAL;
               goto set_threshold_exit;
       }

       tsens_dev.sensor_num = sensor_get_id(sensor_type);
       ...
       while (i < num_thresh) {
               ...
               case THERMAL_TRIP_CONFIGURABLE_HI:
                       if (threshold[i].temp >= temp) {
                               ret = sensor_set_trip(sensor_get_id(sensor_type),
                                       &threshold[i]);
                               if (ret != 0) {
                                       pr_err("%s: Error in setting High trip\n",
                                       KBUILD_MODNAME);
                                       goto set_threshold_exit;
                               }
                               
                               ret = sensor_activate_trip(
                                      (sensor_get_id(sensor_type)),
                                      &threshold[i], true);
                               
				if (ret != 0) {
					pr_err("%s: Error in enabling High trip\n",
						KBUILD_MODNAME);
					goto set_threshold_exit;
				}
                       }		
               ...
               case THERMAL_TRIP_CONFIGURABLE_LOW:
                       if (threshold[i].temp <= temp) {
                               ret = sensor_set_trip(sensor_get_id(sensor_type),
                                       &threshold[i]);
                               if (ret != 0) {
                                       pr_err("%s: Error in setting Low trip\n",
                                       KBUILD_MODNAME);
                                       goto set_threshold_exit;
                               }
				...
				ret = sensor_activate_trip(
					...
					&threshold[i], true);
				if (ret != 0) {
					pr_err("%s: Error in enabling Low trip\n",
					KBUILD_MODNAME);
					goto set_threshold_exit;
				}
                       }
...
{
       int ret = 0, cpu = 0;
       uint32_t max_freq_req = 0;
       uint32_t min_freq_req = 0;
       ...
       while (!kthread_should_stop()) {
               ...
               for_each_possible_cpu(cpu) {
                       ...
                       max_freq_req = min(
                       cpus[cpu].freq_mitig_data.max_freq_limit,
                       cpus[cpu].freq_mitig_data.user_max_freq_limit);

                       min_freq_req = max(
                       cpus[cpu].freq_mitig_data.min_freq_limit,
                       cpus[cpu].freq_mitig_data.user_min_freq_limit);

                       mutex_unlock(
                       &cpus[cpu].freq_mitig_data.freq_mitig_mutex);

                       if (max_freq_req == per_cpu(limited_max_freq, cpu)
                               && min_freq_req ==
                               per_cpu(limited_min_freq, cpu))
                               continue;

                       per_cpu(limited_max_freq, cpu) = max_freq_req;
                       per_cpu(limited_min_freq, cpu) = min_freq_req;
                       update_cpu_freq(cpu);
               }
       }
       return ret;
       ...
       int temp, void *data)
{
       ...
       mutex_lock(&cpu_node->freq_mitig_data.freq_mitig_mutex);
       switch (type) {
       case THERMAL_TRIP_CONFIGURABLE_HI:
               if (cpu_node->freq_mitig_data.max_freq_limit
                       == UINT_MAX) {
                       pr_info("%s: Mitigating cpu %d frequency to %d\n",
                               KBUILD_MODNAME, cpu_node->cpu,
                               msm_thermal_info.freq_mitig_value);

                       cpu_node->freq_mitig_data.max_freq_limit =
                                msm_thermal_info.freq_mitig_value;
                       new_req = 1;
               }
               break;
       case THERMAL_TRIP_CONFIGURABLE_LOW:
               if (cpu_node->freq_mitig_data.max_freq_limit !=
                       UINT_MAX) {
                       pr_info("%s: Removing Kernel frequency mitigation for cpu %d\n",
                               KBUILD_MODNAME, cpu_node->cpu);
       ...
       mutex_unlock(&cpu_node->freq_mitig_data.freq_mitig_mutex);
...
int msm_thermal_set_frequency(struct cpu_freq_arg inp_req, bool max_freq)
{
       int ret = 0;

       mutex_lock(&cpus[inp_req.cpu_num].freq_mitig_data.freq_mitig_mutex);
       if (max_freq) {
               if (cpus[inp_req.cpu_num].freq_mitig_data.user_max_freq_limit
                       == inp_req.freq_req)
                       goto SET_FREQ_EXIT;

               cpus[inp_req.cpu_num].freq_mitig_data.user_max_freq_limit =
                       inp_req.freq_req;
       } else {
               if (cpus[inp_req.cpu_num].freq_mitig_data.user_min_freq_limit
                       == inp_req.freq_req)
                       goto SET_FREQ_EXIT;

               cpus[inp_req.cpu_num].freq_mitig_data.user_min_freq_limit =
                       inp_req.freq_req;
       }
...
SET_FREQ_EXIT:
       mutex_unlock(&cpus[inp_req.cpu_num].freq_mitig_data.freq_mitig_mutex);
       return ret;
}
#endif
...
static void freq_mitigation_init(void)
{
       ...
       for_each_possible_cpu(cpu) {
               ...
               cpus[cpu].cpu = cpu;
               cpus[cpu].freq_mitig_data.max_freq_limit = UINT_MAX;
               cpus[cpu].freq_mitig_data.user_max_freq_limit = UINT_MAX;
               cpus[cpu].freq_mitig_data.min_freq_limit = 0;
               cpus[cpu].freq_mitig_data.user_min_freq_limit = 0;
               mutex_init(&cpus[cpu].freq_mitig_data.freq_mitig_mutex);

               if (!(msm_thermal_info.freq_mitig_control_mask & BIT(cpu)))
                       continue;
               cpus[cpu].freq_mitig_data.thresh[0].temp =
                       msm_thermal_info.freq_mitig_temp_degc;
               cpus[cpu].freq_mitig_data.thresh[0].trip =
                       THERMAL_TRIP_CONFIGURABLE_HI;
               cpus[cpu].freq_mitig_data.thresh[0].notify =
                       freq_mitigation_notify;
               cpus[cpu].freq_mitig_data.thresh[0].data = (void *)&cpus[cpu];

               cpus[cpu].freq_mitig_data.thresh[1].temp =
                       msm_thermal_info.freq_mitig_temp_degc -
                       msm_thermal_info.freq_mitig_temp_hysteresis_degc;
               cpus[cpu].freq_mitig_data.thresh[1].trip =
                       THERMAL_TRIP_CONFIGURABLE_LOW;
               cpus[cpu].freq_mitig_data.thresh[1].notify =
                       freq_mitigation_notify;
...
static void __ref disable_msm_thermal(void)
{
	...
	flush_scheduled_work();
       ...
       for_each_possible_cpu(cpu) {
               if (per_cpu(limited_max_freq, cpu) == UINT_MAX &&
                       per_cpu(limited_min_freq, cpu) == 0)
                       continue;
               per_cpu(limited_max_freq, cpu) = UINT_MAX;
               per_cpu(limited_min_freq, cpu) = 0;
               reset_freq = 1;
       }
       if (!reset_freq)
               ...
               struct platform_device *pdev)
{
       ...
       ret = of_property_read_u32(node, key, &data->freq_mitig_value);
       if (ret)
               goto PROBE_FREQ_EXIT;

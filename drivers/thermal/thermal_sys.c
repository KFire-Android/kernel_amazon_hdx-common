...
static void update_current_threshold(struct sensor_info *sensor)
{
        int i = 0;
        enum thermal_trip_type type;

        for (; ((sensor->max_idx == -1) || (sensor->min_idx == -1)) &&
...
static void __update_sensor_thresholds(struct sensor_info *sensor)
{
        ...
        if ((sensor->max_idx == -1) || (sensor->min_idx == -1))
                update_current_threshold(sensor);
        ...
        if (max_of_low_thresh == LONG_MIN)
                sensor->tz->ops->activate_trip_type(sensor->tz,
                        sensor->min_idx, THERMAL_TRIP_ACTIVATION_DISABLED);
        else
                sensor->tz->ops->activate_trip_type(sensor->tz,
                        sensor->min_idx, THERMAL_TRIP_ACTIVATION_ENABLED);

        if (min_of_high_thresh == LONG_MAX)
                sensor->tz->ops->activate_trip_type(sensor->tz,
                        sensor->max_idx, THERMAL_TRIP_ACTIVATION_DISABLED);
        else
                sensor->tz->ops->activate_trip_type(sensor->tz,
                        sensor->max_idx, THERMAL_TRIP_ACTIVATION_ENABLED);
}
		...
		enum thermal_trip_type trip, long temp)
{
	...
	list_for_each_entry_safe(pos, var, &tz->sensor.threshold_list, list) {
		if ((pos->trip != trip) && (pos->enabled == 0))
			continue;
	...
	struct sensor_threshold *threshold, bool enable)
{
	...
	struct sensor_threshold *pos, *var;
        ...
        if (!sensor || !threshold) {
                pr_err("%s: Invalid or uninitialized data\n",
                        KBUILD_MODNAME);
                ret = -ENODEV;
	...
	/*Schedule the work queue for updating the thresholds*/
...
{
	struct thermal_zone_device *tz = to_thermal_zone(dev);
	int trip = 0, result = 0, trip_index = 0;
	enum thermal_trip_type type;

	if (!tz->ops->get_trip_type) {
		result = -EPERM;
		goto user_activate_exit;
	}
	...
	if (!strncmp(buf, "enabled", sizeof("enabled"))) {
		tz->tz_threshold[trip_index].enabled = 1;
	} else if (!strncmp(buf, "disabled", sizeof("disabled"))) {
		tz->tz_threshold[trip_index].enabled = 0;
	} else {
		result = -EINVAL;
		goto user_activate_exit;
	}

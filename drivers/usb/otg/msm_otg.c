...
{
	static int last_chg_type = USB_INVALID_CHARGER;
	int charger_type;
	 ...
	 */
	if (last_chg_type == motg->chg_type)
		return 0;
	last_chg_type = motg->chg_type;
		...
		if (test_bit(MHL, &motg->inputs)) {
			...
			case USB_CHG_STATE_DETECTED:
				...
				case USB_SDP_CHARGER:
					/* HACK: this allows charging mode to work properly with
					 * computer USB -- may be inappropriate or against spec */
					msm_otg_notify_charger(motg,
							IDEV_CHG_MIN);
					msm_otg_start_peripheral(otg, 1);
					otg->phy->state =

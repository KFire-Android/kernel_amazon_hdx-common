...
static inline void mdss_dsi_panel_platform_delay(void)
{
	msleep(100);
}
#else
...
static int mdss_dsi_regulator_init(struct platform_device *pdev)
{
	...
	if (ctrl_pdata->power_data.num_vreg > 0) {
		...
		ret = regulator_set_voltage(dsi_drv->vdd_io_vreg,
						1800000, 1800000);
		if (ret) {
		...
		ret = regulator_set_voltage(dsi_drv->vdda_vreg,
						1200000, 1200000);
		if (ret) {
			pr_err("%s: set voltage failed on vdda vreg, rc=%d\n",
					__func__, ret);
...
*/
		...
		if (ctrl_pdata->power_data.num_vreg > 0) {
			...
			ret = regulator_disable(
				(ctrl_pdata->shared_pdata).vdda_vreg);
			if (ret) {
				pr_err("%s: Failed to disable regulator.rc=%d\n",
					__func__, ret);
				return ret;
			}
			      ...
			      struct mdss_panel_common_pdata *panel_data)
{
	...
	if (!gpio_is_valid(ctrl_pdata->disp_en_gpio)) {
		...
		if (rc) {
            ...
            ctrl_pdata->disp_lcd_en_gpio = of_get_named_gpio(pdev->dev.of_node,
        ...
        pr_err("request lcd enable gpio ok, = %d \n", ctrl_pdata->disp_lcd_en_gpio);
            ...
            ctrl_pdata->mbist_gpio = of_get_named_gpio(pdev->dev.of_node,
        ...
        pr_err("request lcd enable gpio ok, = %d \n", ctrl_pdata->mbist_gpio);
            ...
            if (!gpio_is_valid(ctrl_pdata->disp_lcd_en_gpio)) {
                ...
                rc = gpio_direction_output(ctrl_pdata->disp_lcd_en_gpio, 1);
                if (rc) {
                    pr_err("set_direction for disp_lcd_en gpio failed, rc=%d\n",
                           rc);
                    gpio_free(ctrl_pdata->disp_lcd_en_gpio);
                    return -ENODEV;
                }
	...
	ctrl_pdata->psr_on_cmds = panel_data->psr_on_cmds;
	ctrl_pdata->psr_off_cmds = panel_data->psr_off_cmds;
	...
	if (panel_data->panel_info.pdest == DISPLAY_1)
		mdss_debug_register_base("dsi0", ctrl_pdata->ctrl_base, 0x600);

	ctrl_pdata->on = panel_data->on;
	ctrl_pdata->off = panel_data->off;

	ctrl_pdata->psr_ctrl = panel_data->psr_ctrl;

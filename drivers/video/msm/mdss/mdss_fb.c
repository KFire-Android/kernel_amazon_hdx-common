...
static int mdss_fb_release_all(struct fb_info *info, bool release_all)
{
	...
	if (!mfd->ref_cnt) {
		mutex_lock(&mfd->lock);
		mfd->is_commit_allowed = false;
		mutex_unlock(&mfd->lock);

		if (flush_work(&mfd->commit_work))
			pr_debug("flushed commit before FB_BLANK\n");
		...
		struct mdp_display_commit *disp_commit)
{
	...
	mutex_lock(&mfd->lock);
	if (!mfd->is_commit_allowed) {
		pr_err("commit is not allowed. Return error\n");
		mutex_unlock(&mfd->lock);
		mutex_unlock(&mfd->sync_mutex);
		return -EPERM;
	}
...
int mdss_fb_add_touch_pm_calls(
...
EXPORT_SYMBOL(mdss_fb_add_touch_pm_calls);
...
int mdss_fb_rm_touch_pm_calls(void)
{
	touch_suspend = NULL;
	touch_resume = NULL;
...
EXPORT_SYMBOL(mdss_fb_rm_touch_pm_calls);
	...
	struct mdss_panel_data *pdata)
{
	...
	if (mdp_instance->panel_register_done)
		mdp_instance->panel_register_done(pdata, split_display_enabled);

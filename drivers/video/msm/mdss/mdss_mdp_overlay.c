...
static inline int mdss_mdp_ad_is_supported(struct msm_fb_data_type *mfd)
{
	...
	if (mixer && (mixer->num > ctl->mdata->nad_cfgs)) {
		if (!mixer)
			...
			pr_warn("mixer attached (%d) doesnt support ad\n", mixer->num);
		return 0;
	}
...
static DEVICE_ATTR(ad, S_IRUGO | S_IWUGO, mdss_mdp_ad_show, mdss_mdp_ad_store);
...
static int mdss_mdp_overlay_splash_parse_dt(struct msm_fb_data_type *mfd)
{
	...
	if (len < 1) {
		pr_err("Error from prop qcom,memblock-reserve : spec error in 	device tree\n");
		return 0;
	}
	...
	pr_debug("%s %d : memaddr=%x size=%x\n", __func__, __LINE__,
			mdp5_mdata->splash_mem_addr, mdp5_mdata->splash_mem_size);
...
static int mdss_mdp_overlay_fb_parse_dt(struct msm_fb_data_type *mfd)
{
	...
	mdss_mdp_overlay_splash_parse_dt(mfd);

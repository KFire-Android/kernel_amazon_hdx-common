 ...
 */
static noinline int do_wp_page(struct mm_struct *mm, struct vm_area_struct *vma,
	...
	if (vma->vm_ops) {
		...
		if (pathbuf) {
			path = d_path(&vma->vm_file->f_path, 
				      ...
				      PATH_MAX + 11);
			if (!IS_ERR(path)) {
				if (strstr(path, "mmap_alloc"))
					detail.call_site
					  = (void *)pageinfo_mmap_alloc_cow;
				else if (strstr(path, "sys_alloc"))
					detail.call_site
					  = (void *)pageinfo_sys_alloc_cow;
				else if (strstr(path, "create_mspace"))
					detail.call_site
					  = (void *)pageinfo_create_mspace_cow;
				else if (strstr(path, "dalvik-heap"))
					detail.call_site
					  = (void *)pageinfo_dalvik_heap_cow;
				else if (strstr(path, "dalvik-LinearAlloc"))
					detail.call_site
					  = (void *)pageinfo_dalvik_linear_cow;
				else if (strstr(path, "://"))
					detail.call_site
				      = (void *)pageinfo_ashmem_unpinnable_cow;
				else if (strstr(path, "dalvik-hashmem"))
					detail.call_site
					  = (void *)pageinfo_dalvik_hashmem_cow;
				else if (strstr(path, "native-hashmem"))
					detail.call_site
					  = (void *)pageinfo_native_hashmem_cow;
				else if (strstr(path, "dalvik-aux-structure"))
					detail.call_site
				    = (void *)pageinfo_dalvik_aux_structure_cow;
				else if (strstr(path, "dalvik-bitmap"))
					detail.call_site
					= (void *)pageinfo_dalvik_gc_noise_cow;
				else if (strstr(path, "dalvik-card-table"))
					detail.call_site
				      = (void *)pageinfo_dalvik_card_table_cow;
				else if (strstr(path, "dalvik-mark-stack"))
					detail.call_site
				      = (void *)pageinfo_dalvik_mark_stack_cow;
				else if (strstr(path, "dalvik-jit-code-cache"))
					detail.call_site
					= (void *)pageinfo_dalvik_jit_cow;
			}
			kfree(pathbuf);
		}
		new_page->detail = detail;
					    ...
					    struct zap_details *details)
{
	...
	list_for_each_entry(vma, head, shared.vm_set.list) {
		...
		unmap_mapping_range_vma(vma, vma->vm_start,
					vma->vm_end, details);
	}
}
 ...
 * We return with mmap_sem still held, but pte unmapped and unlocked.
 */
static noinline int do_swap_page(struct mm_struct *mm,
				 struct vm_area_struct *vma,
 ...
 * We return with mmap_sem still held, but pte unmapped and unlocked.
 */
static noinline int do_anonymous_page(struct mm_struct *mm, struct vm_area_struct *vma,
		unsigned long address, pte_t *page_table, pmd_t *pmd,
		unsigned int flags)
{
	struct page *page;
...
/* ACOS_MOD_BEGIN */
	...
	if (vma->vm_ops) {
		...
		if (pathbuf) {
			...
			if (!IS_ERR(path)) {
			    if (strstr(path, "mmap_alloc"))
				detail.call_site = (void *)pageinfo_mmap_alloc;
			    else if (strstr(path, "sys_alloc"))
				detail.call_site = (void *)pageinfo_sys_alloc;
			    else if (strstr(path, "create_mspace"))
			     detail.call_site = (void *)pageinfo_create_mspace;
			    else if (strstr(path, "dalvik-heap"))
			      detail.call_site = (void *)pageinfo_dalvik_heap;
			    else if (strstr(path, "dalvik-LinearAlloc"))
			      detail.call_site = (void *)pageinfo_dalvik_linear;
			    else if (strstr(path, "://"))
			 detail.call_site = (void *)pageinfo_ashmem_unpinnable;
			    else if (strstr(path, "dalvik-hashmem"))
		         detail.call_site = (void *)pageinfo_dalvik_hashmem;
			    else if (strstr(path, "native-hashmem"))
			 detail.call_site = (void *)pageinfo_native_hashmem;
			    else if (strstr(path, "dalvik-aux-structure"))
		      detail.call_site = (void *)pageinfo_dalvik_aux_structure;
			    else if (strstr(path, "dalvik-bitmap"))
		      detail.call_site = (void *)pageinfo_dalvik_gc_noise;
			    else if (strstr(path, "dalvik-card-table"))
		      detail.call_site = (void *)pageinfo_dalvik_card_table;
			    else if (strstr(path, "dalvik-mark-stack"))
		      detail.call_site = (void *)pageinfo_dalvik_mark_stack;
			    else if (strstr(path, "dalvik-jit-code-cache"))
			      detail.call_site = (void *)pageinfo_dalvik_jit;
			}
			kfree(pathbuf);
		}
		page->detail = detail;
...
/* ACOS_MOD_BEGIN */
#ifdef CONFIG_TRAPZ_PVA
		{
			...
			if (vma->vm_ops->page_mkwrite)
				detail.call_site = (void *)vma->vm_ops->page_mkwrite;
			else
...
static noinline int do_linear_fault(struct mm_struct *mm, struct vm_area_struct *vma,
		unsigned long address, pte_t *page_table, pmd_t *pmd,
		unsigned int flags, pte_t orig_pte)
{
 ...
 * We return with mmap_sem still held, but pte unmapped and unlocked.
 */
static noinline int do_nonlinear_fault(struct mm_struct *mm, struct vm_area_struct *vma,
		unsigned long address, pte_t *page_table, pmd_t *pmd,
		unsigned int flags, pte_t orig_pte)
{
	pgoff_t pgoff;

...
{
	int i;
	for (i = 0; i < sizeof(boring_name_nodes)/sizeof(struct boring_name); ++i) {
...
static int is_boring(unsigned long call_site)
{
	...
	hash_for_each_possible(boring_call_sites, cand, node, hlist, call_site) {
		if (cand->call_site == call_site)
			return 1;
	}
	hash_for_each_possible(nonboring_call_sites, cand, node, hlist, call_site) {
		if (cand->call_site == call_site)
			return 0;
	}

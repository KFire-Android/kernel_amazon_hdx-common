...
static int is_greedy_call_site(unsigned long call_site)
{
	...
	hash_for_each_possible(nongreedy_call_sites, cand, node, hlist, call_site) {
		if (cand->call_site == call_site)
			return 0;
	}
	hash_for_each_possible(greedy_call_sites, cand, node, hlist, call_site) {
		if (cand->call_site == call_site)
			return 1;
	}
					...
					pg_data_t *pgdat, struct zone *zone)
{
	...
	seq_printf(m, "PFNs 0x%05lX (0x%08lX) to 0x%05lX (0x%08lX)\n",
	           start_pfn, (unsigned long)pfn_to_page(start_pfn), end_pfn-1, (unsigned long)pfn_to_page(end_pfn-1));
	...
	for (pfn = start_pfn; pfn < end_pfn; pfn++) {
		...
		if (!pfn_valid(pfn)) {
			if (bad_pfn_start != 0xFFFFFFFF) {
				seq_printf(m, "Valid, not free, no call_site:\t0x%05lX to 0x%05lX-1\n", bad_pfn_start, pfn);
			}
			bad_pfn_start = pfn;
			invalid_pfns++;
		...
		if (PageReserved(page) && page_count(page) && !(((unsigned long)detail.call_site) & ~1)) {
			if (bad_pfn_start != 0xFFFFFFFF) {
				seq_printf(m, "Valid, not free, no call_site:\t0x%05lX to 0x%05lX-1\n", bad_pfn_start, pfn);
			}
			bad_pfn_start = pfn;
			reserved_pfns++;
			while (pfn+1 < end_pfn && pfn_valid(pfn+1) && (page = pfn_to_page(pfn+1)) && memmap_valid_within(pfn+1, page, zone) && PageReserved(page) && page_count(page) && !(((unsigned long)page->detail.call_site) & ~1)) {
				pfn++;
				reserved_pfns++;
			}
		...
		if (((unsigned int)detail.call_site) & 1) {
			...
			if (order >= MAX_ORDER) {
				seq_printf(m, "Nasty race while reading allocation order at pfn %05lX\n", pfn+1);
				order = 0;
			}
			pfn += (1 << order) - 1;
		}
		...
		cs = NULL;
		hash_for_each_possible(pages_by_cs, cand, node, hlist, call_site) {
			if (cand->call_site == call_site
	...
	total_byorder = 0;
	total_bysite = 0;
	seq_printf(m, "Node %2d, zone %8s, span %ld, invalid %ld, holes %ld, reserved %ld\nIn use:  ", pgdat->node_id, zone->name, zone->spanned_pages, invalid_pfns, hole_pfns, reserved_pfns);
	for (order = 0; order < MAX_ORDER; order++) {
...
{
	return seq_open(file, &pageinfo_op);
}

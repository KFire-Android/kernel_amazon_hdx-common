...
#ifdef CONFIG_MACH_URSA
#define WCD9XXX_CS_MEAS_DELTA_MAX_MV 25
#else
#define WCD9XXX_CS_MEAS_DELTA_MAX_MV 12
#endif

static int swch_irq_debounce_time_us = SWCH_IRQ_DEBOUNCE_TIME_US;
static int wcd9xxx_v_cs_hs_max = WCD9XXX_V_CS_HS_MAX;
static int wcd9xxx_v_cs_no_mic = WCD9XXX_V_CS_NO_MIC;
static int wcd9xxx_cs_meas_delta_max_mv = WCD9XXX_CS_MEAS_DELTA_MAX_MV;
...
#ifdef CONFIG_MACH_URSA
	/* Note: Hack to accomodate TTY and other high-impedance type
	 * devices.  Changing the V_HS_MAX to accomodate those
	 * devices has undesired consequences that are more risky
	 * to fix at this point.
	 */
	if (SND_JACK_LINEOUT == status) {
		snd_soc_jack_report_no_dapm(jack, SND_JACK_HEADSET, mask);
	} else {
...
#ifdef CONFIG_SND_HEADSET_TS3A225E
	/* Note: Direct cast to headset_plug_type_t, which maps directly
		to wcd9xxx_mbhc_plug_type type
	 */
	result = ts3a225e_headset_switch_set_type((headset_plug_type_t)plug_type);
	if (result) {
		pr_err("%s - Failed to set the headset switch for headset type %d.  Error code = %d\n", __func__, plug_type, result);
	}
#endif
...
#ifdef CONFIG_SND_HEADSET_TS3A225E
	/* Reset to CTIA MODE */
	result = ts3a225e_headset_switch_set_type(INPUT_TYPE_HEADSET);
	if (result) {
		pr_err("%s - Failed to set the headset switch for headset type %d.  Error code = %d\n", __func__, INPUT_TYPE_HEADSET, result);
	}
#endif
...
#ifdef CONFIG_MACH_URSA
static bool wcd9xxx_is_voltage_valid_for_button(const struct wcd9xxx_mbhc *mbhc,
				  const s32 micmv, int button) {

	s16 *v_btn_low, *v_btn_high;
	struct wcd9xxx_mbhc_btn_detect_cfg *btn_det;
	if (NULL == mbhc) {
		pr_err("Invalid MBHC configuration.");
		return false;
	}
	...
	if (button < 0 || button >= btn_det->num_btn) {
		return false;
	}

	if ((v_btn_low[button] <= micmv) && (v_btn_high[button] >= micmv)) {
		return true;
	}
...
#ifdef CONFIG_MACH_URSA
			bool is_valid_btn_match[2];
			int second_btn_value;
#endif
...
#ifdef CONFIG_MACH_URSA
			/* Just do a direct check if the input voltage for the
			 * second check matched the voltage for the button
			 * we've already determined.  This is more sound logic
			 * for when there's an overlap of button voltage values.
			 */
			second_btn_value = wcd9xxx_determine_button(mbhc, stamv_s);

			is_valid_btn_match[0] = wcd9xxx_is_voltage_valid_for_button(
				mbhc, stamv_s, btn);
			is_valid_btn_match[1] = wcd9xxx_is_voltage_valid_for_button(
				mbhc, stamv_s, second_btn_value);

			if (is_valid_btn_match[0]) {
				btn = second_btn_value;
			}

			if (!is_valid_btn_match[0] && !is_valid_btn_match[1]) {
				btn = -1;
			}
#else
	...
	for (meas = 1; (n_btn_meas && d->n_btn_meas &&
		...
		if (meas + 1 >= d->n_btn_con) {
			for (i = 0; i < d->n_btn_con; i++) {
				...
				is_valid_btn_match[0] = wcd9xxx_is_voltage_valid_for_button(
					mbhc, mv_s[meas], btnmeas[meas - i]);
				is_valid_btn_match[1] = wcd9xxx_is_voltage_valid_for_button(
					mbhc, mv_s[meas -i], btnmeas[meas]);
...
ssize_t hsj_debug_read(struct file *file, char __user *buf,
			      size_t count, loff_t *pos)
{
	const int size = 128;
	char buffer[size];
	...
	int n = 0;
	struct wcd9xxx_mbhc *mbhc = file->private_data;
	n = scnprintf(buffer, size, "gpio_level_insert = %d\n",
		mbhc->mbhc_cfg->gpio_level_insert);

	n += scnprintf(buffer2, size, "debounce_time_us = %d\n",
		swch_irq_debounce_time_us);

	strcat(buffer, buffer2);
	n += scnprintf(buffer2, size, "v_cs_hs_max = %d\n",
		wcd9xxx_v_cs_hs_max);

	strcat(buffer, buffer2);
	n += scnprintf(buffer2, size, "v_cs_no_mic = %d\n",
		wcd9xxx_v_cs_no_mic);

	strcat(buffer, buffer2);
	n += scnprintf(buffer2, size, "delta_max_mv = %d\n",
		wcd9xxx_cs_meas_delta_max_mv);
...
static ssize_t hsj_debug_write(struct file *filp,
				 const char __user *ubuf, size_t cnt,
				 loff_t *ppos)
{
	char lbuf[32];
	...
	buf = (char *)lbuf;
        ...
        num_values = sscanf(buf, "%s %d", field_name, &field_value);
	if (num_values < 2) {
		return -EINVAL;
	}

        if (!strcmp(field_name, "gpio_level_insert")) {
		mbhc->mbhc_cfg->gpio_level_insert = field_value ? 1 : 0;
		if (mbhc->mbhc_cfg->gpio_level_insert)
			snd_soc_write(mbhc->codec, WCD9XXX_A_MBHC_INSERT_DETECT,
				      (0x68 | (1 << 1)));
		else
		...
		/* Re-enable detection */
		snd_soc_update_bits(mbhc->codec, WCD9XXX_A_MBHC_INSERT_DETECT, 1, 1);
		} else if (!strcmp(field_name, "debounce_time_us")) {
			swch_irq_debounce_time_us = field_value > 0 ? field_value :
				SWCH_IRQ_DEBOUNCE_TIME_US;
		} else if (!strcmp(field_name, "v_cs_hs_max")) {
			wcd9xxx_v_cs_hs_max = field_value > 0 ? field_value :
				WCD9XXX_V_CS_HS_MAX;
		} else if (!strcmp(field_name, "v_cs_no_mic")) {
			wcd9xxx_v_cs_no_mic = field_value > 0 ? field_value :
				WCD9XXX_V_CS_NO_MIC;
		} else if (!strcmp(field_name, "delta_max_mv")) {
			wcd9xxx_cs_meas_delta_max_mv = field_value > 0 ? field_value :
				WCD9XXX_CS_MEAS_DELTA_MAX_MV;
		} else {
		return -EINVAL;
	}
...
static const struct file_operations hsj_debug_ops = {
	.open = codec_debug_open,
	.read = hsj_debug_read,
	.write = hsj_debug_write,
};

ssize_t btn_det_debug_read(struct file *file, char __user *buf,
			      size_t count, loff_t *pos)
{
	const int size = 512;
	char buffer[size];
	...
	int i = 0;
	struct wcd9xxx_mbhc *mbhc = file->private_data;
	struct wcd9xxx_mbhc_btn_detect_cfg *btn_det = NULL;
	void *calibration = NULL;
	u16 * btn_low = NULL;
	u16 * btn_high = NULL;

	if (NULL == file || NULL == mbhc) {
		return -EINVAL;
	}

	if (NULL == mbhc->mbhc_cfg) {
		return -EINVAL;
	}
	...
	btn_low = (u16 *)wcd9xxx_mbhc_cal_btn_det_mp(btn_det, MBHC_BTN_DET_V_BTN_LOW);
	btn_high = (u16 *)wcd9xxx_mbhc_cal_btn_det_mp(btn_det, MBHC_BTN_DET_V_BTN_HIGH);

	if (NULL == btn_low || NULL == btn_high) {
		return -EINVAL;
	}

	for (i = 0; i < btn_det->num_btn; i++) {
		n += scnprintf(&(buffer[n]), size - n, "btn[ %d ] %03d %03d\n",
			i, btn_low[i], btn_high[i]);
	}

	return simple_read_from_buffer(buf, count, pos, buffer, n);
}

static ssize_t btn_det_debug_write(struct file *file,
				 const char __user *ubuf, size_t cnt,
				 loff_t *ppos)
{
	struct wcd9xxx_mbhc *mbhc = file->private_data;
	...
	buf = (char *)lbuf;
        ...
        num_values = sscanf(buf, "btn[ %d ] %d %d\n",
		&btn_number, &btn_low_value, &btn_high_value);

	while (3 == num_values) {
		if (btn_number >= 0 && btn_number < btn_det->num_btn) {
			btn_low[btn_number] = (u16)btn_low_value;
			btn_high[btn_number] = (u16)btn_high_value;
		}
...
static const struct file_operations btn_det_debug_ops = {
	.open = codec_debug_open,
	.read = btn_det_debug_read,
	.write = btn_det_debug_write,
};
...
#if defined(CONFIG_MACH_URSA)
	mbhc->debugfs_hsj =
	    debugfs_create_file("wcd9xxx_hsj", S_IFREG | S_IRUGO,
				NULL, mbhc, &hsj_debug_ops);
	mbhc->debugfs_btn_det =
	    debugfs_create_file("wcd9xxx_btn_det", S_IFREG | S_IRUGO,
				NULL, mbhc, &btn_det_debug_ops);
#endif
}
...
#if defined(CONFIG_MACH_URSA)
	debugfs_remove(mbhc->debugfs_hsj);
	debugfs_remove(mbhc->debugfs_btn_det);
#endif
}
#else
static void wcd9xxx_init_debugfs(struct wcd9xxx_mbhc *mbhc)
...
#ifdef CONFIG_SND_HEADSET_TS3A225E
        headset_result = ts3a225e_headset_switch_init();
        if (headset_result) {
            pr_err("%s: Failed to initialize ts3a225e switch.  Error code = %d\n", __func__, headset_result);
            /* Note: Unlike other parts of the initilization, nothing special
                     to do at the end here if there's a failure.
             */
        }
#endif
...
#ifdef CONFIG_SND_HEADSET_TS3A225E
        ts3a225e_headset_switch_deinit();
#endif
}

...
#ifndef CONFIG_SND_SOC_MAX97236
...
static void msm8974_liquid_docking_irq_work(struct work_struct *work)
{
	...
	dock_plug_det = gpio_get_value(dock_dev->dock_plug_gpio);
...
static struct snd_soc_jack_pin max97236_headset_pins[] = {
	{
		.pin    = "Headset Mic",
...
static int msm8974_asoc_machine_late_probe(struct snd_soc_card *card)
{
	...
	msm_snd_enable_max97236_ext_clk(codec);

	ret = snd_soc_jack_new(codec_max97236, "JACK",
			SND_JACK_HEADSET | SND_JACK_LINEOUT,
			&max97236_headset);

	snd_jack_set_key(max97236_headset.jack, SND_JACK_BTN_0 | SND_JACK_BTN_2 | SND_JACK_BTN_4, KEY_MEDIA);
#if 0
	ret = snd_soc_jack_add_pins(&max97236_headset,
			ARRAY_SIZE(max97236_headset_pins),
			max97236_headset_pins);
#endif

	max97236_mic_detect(codec_max97236, &max97236_headset);
...
#ifdef CONFIG_SND_SOC_MAX97236
	.late_probe =  msm8974_asoc_machine_late_probe,
#endif
};
...
#ifndef CONFIG_SND_SOC_MAX97236
	ret = of_property_read_u32(pdev->dev.of_node,
		"qcom,mbhc-gpio-level-insert", &mbhc_gpio_level_insert);
	/* Only update the structure with the value if it is defined.
	 * Its not an error if its not defined, its an optional
	 * override parameter.
	 */
	if (!ret) {
		mbhc_cfg.gpio_level_insert = mbhc_gpio_level_insert;
	}
#endif

					    ...
					    uint16_t op_FR_ip_FR_weight)
{
	...
	if (avail_length < 2 * sizeof(uint32_t)) {
		pr_err("%s: avail_length %d less than twice uint32_t\n",
			__func__, avail_length);
		goto skip_send_cmd;
	}
	...
	*update_params_value32++ = DEFAULT_CHMIXER_PARAM_ID_COEFF;
	...
	if (avail_length < 10 * sizeof(uint16_t)) {
		pr_err("%s: avail_length %d less than ten times uint16_t\n",
			__func__, avail_length);
		goto skip_send_cmd;
	}
	...
	*update_params_value16++ = PCM_CHANNEL_FR;
	...
	if (avail_length < 4 * sizeof(uint16_t)) {
		pr_err("%s: avail_length %d less than four times uint16_t\n",
			__func__, avail_length);
		goto skip_send_cmd;
	}
...
{
	int session_type, path_type, port_id, topology, rc = 0;
	u32 channels;
	uint16_t bits_per_sample = 16;
	...
	for (be_index = 0; be_index < MSM_BACKEND_DAI_MAX; be_index++) {
		...
		if (((port_id == SLIMBUS_0_RX) ||
			...
			for_each_set_bit(i,
				...
				ret = dolby_dap_set_custom_stereo_onoff(
					msm_bedais[be_index].port_id,
					is_custom_stereo_on);
				if (ret < 0)
					pr_err("%s:Set DAP CS err = %d\n",
...
static int msm_pcm_routing_prepare(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	unsigned int be_id = rtd->dai_link->be_id;
	int i, path_type, session_type, port_id, topology, rc = 0;
	struct msm_pcm_routing_bdai_data *bedai;
	u32 channels;
